...
enum TwozzControlShape: Equatable {
  case capsule
  case circle
}
...
extension View {
  @ViewBuilder
  func TwozzControlButtonStyle(shape: TwozzControlShape = .capsule) -> some View {
    modifier(TwozzControlButtonStyleModifier(shape: shape))
  }
  ...
  @ViewBuilder
  func chatSettingsGlassButton(isSelected: Bool = false, shape: TwozzControlShape = .capsule) -> some View {
...
struct ChatSettingsGlassButtonModifier: ViewModifier {
  ...
  func body(content: Content) -> some View {
    if glassDisabled {
      content
        .buttonStyle(TwozzOpaquePillButtonStyle(isSelected: isSelected, shape: shape))
        .focusEffectDisabled()
...
struct TwozzOpaquePillButtonStyle: ButtonStyle {
  var isSelected: Bool
  var shape: TwozzControlShape
...
struct TwozzControlButtonStyleModifier: ViewModifier {
  var shape: TwozzControlShape
  @Environment(\.glassDisabled) private var glassDisabled
  @Environment(\.themePalette) private var palette
  ...
  func body(content: Content) -> some View {
    if glassDisabled {
      content
        .buttonStyle(TwozzOpaqueControlButtonStyle(shape: shape))
      ...
      if palette.isLight {
        ...
        content
          .buttonStyle(TwozzLightGlassControlButtonStyle(shape: shape))
          .focusEffectDisabled()
      } else {
        content.buttonStyle(.glass)
      }
...
struct TwozzOpaqueControlButtonStyle: ButtonStyle {
  var shape: TwozzControlShape
...
@available(tvOS 26.0, *)
struct TwozzLightGlassControlButtonStyle: ButtonStyle {
  var shape: TwozzControlShape
